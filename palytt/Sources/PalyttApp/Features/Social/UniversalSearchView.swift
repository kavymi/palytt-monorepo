...
class UniversalSearchViewModel: ObservableObject {
    ...
    private func searchPosts(query: String) async -> [BackendService.BackendPost] {
        do {
            return try await backendService.searchPosts(query: query, limit: 10)
        } catch {
    ...
    private func searchPlaces(query: String) async -> [BackendService.PlaceSearchResult] {
        do {
            // TODO: Get user location for better place search results
            return try await backendService.searchPlaces(query: query, latitude: nil, longitude: nil, limit: 10)
        } catch {
    ...
    private func searchUsers(query: String) async -> [BackendUser] {
        do {
            return try await backendService.searchUsers(query: query, limit: 10)
        } catch {
